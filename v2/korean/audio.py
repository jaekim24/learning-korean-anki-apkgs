...
Audio is keyed by a hash of the text, so identical text across decks is fetched
once and reused. The cache lives in build/audio/ and persists between runs --
rebuilding a deck costs no network calls unless its text actually changed.
"""
...
CACHE_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "build", "audio")
